...
3. # LENGTH
  ...
  **Can also be used to get the `length of an array`!**<br/>
  Usage:
  ```php
...
4. # CONCAT
  Concatenate multiple fields with a glue between each.<br/>
  Usage:
  ```php
...
5. # POSITION
  Get the position of a sub string.<br/>
  Usage:
  ```php
...
1. # SUM
  Retrieve the sum of a field.<br/>
  ...
  ```
2. # MAX
  Retrieve the max value of a field.<br/>
  Reduces amount of documents in result set to one. (If Group-By is not used)<br/>
  Usage:
  ```php
  ["ALIAS" => ["MAX" => "fieldName"], ...]
  ```
3. # MIN
  Retrieve the min value of a field.<br/>
  Reduces amount of documents in result set to one. (If Group-By is not used)<br/>
  Usage:
  ```php
  ["ALIAS" => ["MIN" => "fieldName"], ...]
